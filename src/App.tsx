...
// Theme Helpers
// =============================================================================
const ACCENT_COLORS: Record<ColorScheme, { name: string; swatch: string }> = {
  indigo: { name: 'Indigo', swatch: '#6366f1' },
  emerald: { name: 'Emerald', swatch: '#10b981' },
  rose: { name: 'Rose', swatch: '#f43f5e' },
  amber: { name: 'Amber', swatch: '#f59e0b' },
};
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        theme: (p.theme && p.theme.mode && p.theme.scheme) ? p.theme : { ...DEFAULT_THEME },
      };
    }
  } catch { /* ignore */ }
  return {
...
function SettingsPanel({ open, onClose, config, setConfig, logs, clearLogs, sessions, setSessions, setActiveSessionId }: SettingsPanelProps) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data.config) {
          ...
          if (data.config.theme) { nc.theme = data.config.theme; applyThemeToDOM(nc.theme); imported.push('theme'); }
  ...
  const tabs: { id: SettingsTab; icon: React.ReactNode; label: string }[] = [
    ...
    { id: 'theme', icon: <IconPalette className="w-4 h-4" />, label: 'Theme' },
